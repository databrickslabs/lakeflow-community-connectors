...
class TestCaseInsensitiveDict:
    """CaseInsensitiveDict — the wrapper applied to Spark options."""

    def _build(self):
        from databricks.labs.community_connector.libs.utils import CaseInsensitiveDict

        # Lowercased keys, as Spark's CaseInsensitiveStringMap delivers them.
        return CaseInsensitiveDict(
            {
                "tablename": "messages",
                "operation": "read_table",
                "databricks.connection": "yyoli_gmail_connection",
            }
        )

    def test_indexing_is_case_insensitive(self):
        d = self._build()
        assert d["tableName"] == "messages"
        assert d["TABLENAME"] == "messages"
        assert d["tablename"] == "messages"

    def test_get_is_case_insensitive(self):
        d = self._build()
        assert d.get("tableName") == "messages"
        assert d.get("TableName") == "messages"
    ...
    def test_contains_is_case_insensitive(self):
        d = self._build()
        assert "tableName" in d
        assert "TABLENAME" in d
        assert "tablename" in d
        assert "missing" not in d

    def test_iteration_preserves_original_keys(self):
        # Connector code that introspects the dict sees the keys Spark
        # actually delivered (lowercased), so behaviour stays predictable.
        d = self._build()
        assert set(d.keys()) == {"tablename", "operation", "databricks.connection"}

    def test_missing_key_raises_with_requested_casing(self):
        from databricks.labs.community_connector.libs.utils import CaseInsensitiveDict
    ...
    def test_cloudpickle_round_trip(self):
        # Spark serialises DataSource state to executors via cloudpickle —
        # which reconstructs dict subclasses by calling __setitem__ on a
        # __new__ instance, bypassing __init__.  Regression guard for the
        # ``'CaseInsensitiveDict' object has no attribute '_index'`` crash.
        cloudpickle = pytest.importorskip("cloudpickle")
        d = self._build()
        restored = cloudpickle.loads(cloudpickle.dumps(d))
        assert type(restored).__name__ == "CaseInsensitiveDict"
        assert restored["tableName"] == "messages"
        assert restored.get("OPERATION") == "read_table"
