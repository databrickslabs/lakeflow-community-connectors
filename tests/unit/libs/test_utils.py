...
class TestCaseInsensitiveDict:
    """Wrapper applied to Spark options at the LakeflowSource boundary."""

    def _build(self):
        from databricks.labs.community_connector.libs.utils import CaseInsensitiveDict

        # Lowercased keys as the user's notebook + UC injection delivered them.
    ...
    def test_iteration_preserves_original_keys(self):
        # Connector code that introspects the dict sees whatever Spark
        # delivered, so behaviour stays predictable.
        d = self._build()
    ...
    def test_cloudpickle_round_trip(self):
        ...
        # __new__-ed instance — bypassing __init__. Regression guard for the
        # earlier ``'CaseInsensitiveDict' object has no attribute '_index'``
        # crash that ate a wheel deploy.
