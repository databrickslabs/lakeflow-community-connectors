...
class TestSPMExtraction:
    def test_celr_spm(self):
        ...
        assert row["specimen_type"]["code"] == "258500001"
        assert row["specimen_type"]["text"] == "Nasopharyngeal swab"

    def test_gc_spm(self):
        msg = parse_first(load_sample("sample_oru_gc_testing.hl7"))
        row = extract_segment(msg, "SPM", _extract_spm)
        assert row["specimen_type"]["code"] == "119393003"

    def test_flu_ar_spm_source_site(self):
        msg = parse_first(load_sample("sample_oru_flu_ar.hl7"))
        row = extract_segment(msg, "SPM", _extract_spm)
        assert row["specimen_type"]["code"] == "258604001"
        assert row["specimen_source_site"]["code"] == "181200003"

    def test_concat_notes_spm(self):
        msg = parse_first(load_sample("sample_oru_concat_notes.hl7"))
        row = extract_segment(msg, "SPM", _extract_spm)
        assert row["specimen_type"]["code"] == "258528007"
        assert row["specimen_type"]["text"] == "rectal swab"
...
class TestSPMNewComposites:
    ...
    def test_spm_eip_cq_and_dr(self):
        ...
        row = _extract_spm(msg.get_segment("SPM"))
        assert row["specimen_id"]["placer_assigned_identifier"]["entity_identifier"] == "SPEC123"
        assert row["specimen_id"]["placer_assigned_identifier"]["namespace_id"] == "NS1"
        assert row["specimen_id"]["placer_assigned_identifier"]["universal_id"] == "OID1"
        assert row["specimen_id"]["placer_assigned_identifier"]["universal_id_type"] == "ISO"
        assert row["specimen_id"]["filler_assigned_identifier"]["entity_identifier"] == "PARENT99"

        assert row["specimen_collection_amount"]["quantity"] == "10.5"
        assert row["specimen_collection_amount"]["units"] == "mL"
