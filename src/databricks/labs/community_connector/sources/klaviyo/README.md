...
| `campaigns` | Email / SMS / mobile-push marketing campaign objects, with audience, scheduling, and tracking settings. | `id` | `updated_at` (per channel) | `cdc` | **Channel fan-out**: Klaviyo *requires* a `messages.channel` filter on every request, so the connector issues one paginated walk per channel (`email`, `sms`, `mobile_push`) and unions results. The cursor is a **per-channel bag** (not a single timestamp). A `channel` column is stamped onto each record. Partitioned by channel across executors. |
...
| `page_size` | all tables | per-endpoint max (`profiles` 100, `events` 1000, `lists` 10, `campaigns` 100, `metrics` 200, `flows` 50, `segments` 10, `templates` 10) | Value sent as `page[size]`. Clamped to the endpoint's documented maximum. |
...
- **`campaigns`** — split by channel. Because Klaviyo *requires* a `messages.channel` filter on every request, the three channel values (`email`, `sms`, `mobile_push`) form a natural three-way partition; each executor walks one channel's cursor pages.
...
- **`campaigns` cursor is a per-channel bag, not a single timestamp.** The saved offset has the shape `{cap, updated_at__email, updated_at__sms, updated_at__mobile_push}`. Each channel advances independently. Downstream consumers shouldn't try to interpret the cursor as a wall-clock checkpoint.
