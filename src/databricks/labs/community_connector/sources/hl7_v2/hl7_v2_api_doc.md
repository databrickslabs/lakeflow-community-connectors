...
| EIP | Entity Identifier Pair | placer assigned identifier (EI) ^ filler assigned identifier (EI) | Modeled as `ARRAY<STRUCT<parent: EI, child: EI>>` via `_eip_array_fields()` / `_eip_array_schema()`. Each EI struct has 4 sub-components (`entity_identifier`, `namespace_id`, `universal_id`, `universal_id_type`). Used by `spm.specimen_id`, `spm.specimen_parent_ids`, `obx.observation_related_specimen`, `obr.parent_placer_order_number`, `obr.parent_order`, `orc.parent_order`. |
