...
- **Non-repeating composite fields** (HL7 cardinality `1`, e.g. PID-8 `administrative_sex`, PV1-19 `visit_number`) are modeled as a single nested `STRUCT` column named `<prefix>` (the composite-type helpers return one key whose value is a dict of the sub-components, or `None` when the composite is absent). Only the first repetition is captured if a sender unexpectedly puts a `~` in a cardinality-1 field; the full raw value is preserved in `raw_segment` for forensic reconstruction.

> **Packaging convention (updated):** Single-occurrence composites are nested `STRUCT<...>` columns and repeating composites are `ARRAY<STRUCT<...>>`; the struct field names are identical between the two forms. The per-type "Connector Extraction" notes in the table below describe the **sub-component set** of each type — those sub-components are now struct fields under the `<prefix>` column (e.g. `administrative_sex.code`, `visit_number.assigning_authority`) rather than flat `<prefix>_<subfield>` sibling columns. The single exception is **DR (date/time range)**, which is kept as two flat `_start` / `_end` string columns. Scalar (non-composite) fields remain plain top-level columns.
