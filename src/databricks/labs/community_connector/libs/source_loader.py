...
def get_register_function(source_name: str):
    ...
    Example:
        ...
        >>> register_fn(spark)
    """
    base_pkg = "databricks.labs.community_connector.sources"
    # Check if the source package exists
    try:
        importlib.import_module(f"{base_pkg}.{source_name}")
    ...
    module_path = f"{base_pkg}.{source_name}._generated_{source_name}_python_source"
    try:
        module = importlib.import_module(module_path)
    except ModuleNotFoundError:
        raise ImportError(
            f"Could not import '_generated_{source_name}_python_source.py' from source '{source_name}'. "
            f"Please ensure 'src/databricks/labs/community_connector/sources/{source_name}/_generated_{source_name}_python_source.py' exists."
        )
